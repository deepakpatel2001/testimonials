...
function updateUser(index) {
    userImage.src = userInfo[index].img;
    userName.textContent = userInfo[index].userName;
    work.textContent = userInfo[index].profession;
    userBio.textContent = userInfo[index].bio;
}

rightArrow.addEventListener('click', () => {
    currentIndex = (currentIndex + 1) % userInfo.length;
    updateUser(currentIndex);
});

leftArrow.addEventListener('click', () => {
    currentIndex = (currentIndex - 1 + userInfo.length) % userInfo.length;
    updateUser(currentIndex);
});

btn.addEventListener('click', () => {
    const randomNumberIndex = Math.floor(Math.random() * userInfo.length);
    updateUser(randomNumberIndex);
});